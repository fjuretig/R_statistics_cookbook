...
data               = read.table("C:\\R_book\\E1.txt",sep="\t",head=T) 
data$home_away     = ifelse(data$ha == "H", 1, 0) 
data$date          = as.Date(data$date,format="%m/%d/%Y") 
data               = data %>% mutate(diff_days = as.numeric(date-lag(date))) 
data[is.na(data)]  = 0  
seatbeltsfit <- tsglm(data$Goals, model = list(past_obs = c(1:3)), link = "log", distr = "poisson",xreg = cbind(data$home_away,data$pos,data$diff_days,data$champions_next_days_after,data$champions_next_days_before)) 
